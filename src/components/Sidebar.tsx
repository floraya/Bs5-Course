...
const renderCategoryIcon = (id: string) => {
  switch (id) {
    ...
    case 'components':
      return <Component className="w-4 h-4 text-amber-400" />;
    case 'helpers':
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  // Auto-scroll current lesson into view when lesson changes
  React.useEffect(() => {
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed lg:sticky top-0 lg:top-[54px] bottom-0 left-0 z-40
          bg-slate-950 border-r border-slate-800/80 flex flex-col
          h-full lg:h-[calc(100vh-54px)] max-h-screen
          transition-all duration-300 ease-in-out shrink-0
          ${
            isOpen
              ? 'w-80 sm:w-84 translate-x-0 opacity-100 shadow-2xl lg:shadow-none'
      ...
      >
        ...
        <div className="w-80 sm:w-84 flex flex-col h-full min-w-[20rem] sm:min-w-[21rem]">
        ...
        {/* Lessons List Navigation (Scrollable with dedicated styled scrollbar) */}
        <div className="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar sidebar-scrollbar overscroll-contain">
          {/* If Search is Active */}
          {filteredLessons ? (
            ...
            <>
              ...
              <div className="px-2.5 py-2 bg-gradient-to-r from-purple-950/40 to-slate-900 border border-purple-800/40 rounded-xl flex items-center justify-between text-[11px] text-purple-300">
                <span className="flex items-center gap-1.5 font-bold">
                  <span>📈 學習路徑</span>
                  <span className="text-[10px] text-purple-400 font-normal">(6 大類別)</span>
                </span>
                <div className="flex items-center gap-2 text-[10px]">
                  <button
                    onClick={expandAll}
                    className="text-purple-300 hover:text-white font-medium hover:underline transition-colors"
                    title="展開所有課程分類"
                  >
                    全部展開
                  </button>
                  ...
                  <button
                    onClick={collapseAll}
                    className="text-slate-400 hover:text-slate-200 font-medium hover:underline transition-colors"
                    title="收合所有課程分類"
                  >
                    全部收合
                  </button>
              ...
              {CATEGORIES.map((cat, catIdx) => {
                ...
                return (
                  <div
                    key={cat.id}
                    className="rounded-xl overflow-hidden border border-slate-800/70 bg-slate-900/40 transition-all shadow-sm"
                  >
                    {/* Category Header */}
                    <button
                      onClick={() => toggleCategory(cat.id)}
                      className={`w-full flex items-center justify-between p-2.5 sm:p-3 text-left transition-colors ${
                        !isCollapsed
                      ...
                      <div className="flex items-center gap-2.5 min-w-0 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between gap-1.5">
                            ...
                            <span
                              className={`text-[9px] px-1.5 py-0.5 rounded border font-mono shrink-0 ${difficultyColor}`}
                            >
                              {cat.difficulty || '初階'}
                            </span>
                          </div>
                          ...
                          <div className="text-[10px] text-slate-400 flex items-center gap-1.5 mt-0.5 whitespace-nowrap">
                            <span className="text-purple-400 font-mono font-medium">階段 {catIdx + 1}</span>
                            <span className="text-slate-600">·</span>
                            <span className="text-slate-400 font-mono">{cat.enName}</span>
                            <span className="text-slate-600">·</span>
                            <span className="font-mono text-slate-300 font-medium">{completedInCat}/{lessons.length}</span>
                            {isAllDoneInCat && (
                              <span className="text-[9px] bg-emerald-500/20 text-emerald-300 px-1 py-0.2 rounded border border-emerald-500/40 font-bold ml-auto">
                                ✓ 完結
                              </span>
                            )}
                          </div>
